...
function execute(window) {
	if (CurrentProcessingInfo.starMaskId == null) {
		ConsoleWriter.writeMessageStart("Missing Star Mask, creating…");
		createProcessedStarMask(CurrentProcessingInfo);
		ConsoleWriter.writeMessageEnd("Missing Star Mask created.");
    }
	...
	ConsoleWriter.writeMessageStart("Running StarReduction");
...
function customizeDialog() {
	GlobalDialog.infoBox.text = "<b>EZ Star Reduction:</b> A script to easily reduce star sizes in a given image. For star mask generation, StarNet++ needs to be installed. If the PixInsight StarNet++ module is not installed, a monochrome StarNet starmask of the image needs to be supplied. The script will automatically create a StarNet starmask if none is supplied.";

	GlobalDialog.tutorialPrerequisites = ["Image is cropped properly", "Image is stretched to non-linear", "StarNet++ is installed or StarNet++ raw star mask is present"];
	GlobalDialog.tutorialSteps = ["Select image to apply to", "Press 'Create mask for reduction' or select a raw mask generated by StarNet++", "Adjust Star Reduction iterations if desired", "Run EZ Star Reduction"];

	GlobalDialog.onEmptyMainView = function() { }

	GlobalDialog.onSelectedMainView = function(view) {
		...
		onViewSelected = function (value) {
			...
			if (value.image.isColor) {
				...
				if(value.image.width != mainView.image.width || value.image.height != mainView.image.height) {
					...
					GlobalDialog.showWarningDialog("Cannot use 'Alt' Images. Those are for only for the script.");
					GlobalDialog.starMaskImageSelector.remove(value);
				} else {
					CurrentProcessingInfo.starMaskId = value.isPreview ? value.fullId : value.id;
					ConsoleWriter.writeMessageBlock("Selected star mask '" + CurrentProcessingInfo.starMaskId + "'");
				}
			}
		}
	...
	GlobalDialog.starMaskImageSizer = new HorizontalSizer();
	...
	with (GlobalDialog.createProcessedStarMaskButton) {
		...
		onClick = function () {
            ...
            GlobalDialog.starMaskImageSelector.currentView = View.viewById(CurrentProcessingInfo.starMaskId);
			JobStack.stopProcessing();
		}
	}

	GlobalDialog.modeLabel = new Label(GlobalDialog);
	with (GlobalDialog.modeLabel) {
		text = "Star Reduction Mode";
		scaledMaxWidth = 130;
	}

	GlobalDialog.modeComboBox = new ComboBox(GlobalDialog);
    with (GlobalDialog.modeComboBox) {
        editEnabled = false;
	...
	GlobalDialog.modeSizer = new HorizontalSizer();
	with (GlobalDialog.modeSizer) {
		addItem(GlobalDialog.modeLabel);
		addItem(GlobalDialog.modeComboBox);
	}  
	...
	GlobalDialog.mainControlReplacement = new GroupBox(GlobalDialog);
	with (GlobalDialog.mainControlReplacement) {
        ...
        sizer.addItem(GlobalDialog.modeSizer);
...
function doStarReduction(view, starMaskWindow, iterations, mode) {
	...
	if(mode == StarReduction_Morph) {
		ConsoleWriter.writeMessageBlock("Reducing Sars with morphological transformation");
		...
		ConsoleWriter.writeMessageBlock("Creating starless image for edge replacement");
		...
		ConsoleWriter.writeMessageBlock("Replacing edges");
...
function createRawStarMask(starReductionInfo) {
	ConsoleWriter.writeMessageStart("Creating raw star mask based on " + starReductionInfo.mainViewId);
	// clone main image
	...
	if (starReductionInfo.downsampleStarnet) {
		ConsoleWriter.writeMessageBlock("Downsampling " + clonedView.id);
	...
	starNet.mask = true;
	starNet.stride = 0;
	ConsoleWriter.writeMessageBlock("Running StarNet++ on " + clonedView.id + " - hold tight, this might take a while.");
	...
	if (starReductionInfo.downsampleStarnet) {
		ConsoleWriter.writeMessageBlock("Upscaling " + clonedView.id);
	...
	ConsoleWriter.writeMessageEnd("Raw Mask Creation done");
}

function createProcessedStarMask(starReductionInfo) {
	ConsoleWriter.writeMessageStart("Creating processed star mask based on " + starReductionInfo.mainViewId);
	createRawStarMask(starReductionInfo);
	processStarMask(starReductionInfo.starMaskId, starReductionInfo.mode);
	ConsoleWriter.writeMessageEnd("Mask processing done");
...
function processStarMask(starMaskId, mode) {
	...
	ConsoleWriter.writeMessageStart("Processing star mask, mode: " + modeString);
	if(mode == StarReduction_Morph) {
	...
	ConsoleWriter.writeMessageEnd("Processing complete");
}
...
function applyPixelMathMorph(viewId) {
	ConsoleWriter.writeMessageBlock("Applying PixelMath to " + viewId);
...
function applyConvolute(viewId, sigma, shape) {
	ConsoleWriter.writeMessageBlock("Applying Convolute to " + viewId);
	let view = View.viewById(viewId);
	// apply convolution
	// sigma 4.5, shape 2
...
function applyBoost(viewId) {
	ConsoleWriter.writeMessageBlock("Applying boost to " + viewId);
	let view = View.viewById(viewId);
...
function applyFinalBoost(viewId) {
	ConsoleWriter.writeMessageBlock("Applying final boost to " + viewId);
	let view = View.viewById(viewId);
	// apply curvestransformation
	doFinalBoost(view);
}
...
function applyBinarize(viewId) {
	ConsoleWriter.writeMessageBlock("Applying binarize to " + viewId);
	let view = View.viewById(viewId);
	doBinarize(view);
}

function applyDilate(viewId) {
	ConsoleWriter.writeMessageBlock("Applying dilate to " + viewId);
	let view = View.viewById(viewId);
	doDilate(view);
}
